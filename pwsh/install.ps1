...
`e[93m$("Please select components you want to install:")`e[0m
...
"@
    Write-Host
    do {
        $selection = Read-Host "Enter the numbers of components you want to install (separated by commas)"
...
WinMac menu is standalone and customizable application that resembles Apple menu.

Classic Start is an enhanced Windows 7 Start menu with modernised looks.
    ...
    #* StartAllBack
        "4" {
            if (!($osVersion -like '*Windows 11*')) {
                ...
                Set-ItemProperty -Path $sabRegPath -Name "Start_SortOverride" -Value 0
                Set-ItemProperty -Path $sabRegPath -Name "StartMetroAppsFolder" -Value 2
...
"@
            ...
            if ($roundedOrSquared -eq "S" -or $roundedOrSquared -eq "s") {
                ...
                if ($lightOrDark -eq "D" -or $lightOrDark -eq "d") {
                    ...
                    $modifiedContent = $modifiedContent | ForEach-Object { $_ -replace "recycle_bin_full_dark", "recycle_bin_full_light" }
...
"@
    ...
    #* Other
        ...
        #? Black Cursor
            # $cursorsArchivePath = (Get-Item -Path "..\config\cursors\windows-modern-v2.zip").FullName
            Expand-Archive -Path '..\config\cursors\windows-modern-v2.zip' -DestinationPath "$ENV:WINDIR\Cursors" -Force
